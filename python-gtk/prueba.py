...
class Mi_Ventana(Gtk.Window):
	...
	def agragar_boton(self):
		...
		self.contenedor.attach_next_to(self.boton3,self.boton2,Gtk.PositionType.BOTTOM,2,2)
		self.boton3.connect('clicked',self.agregar_fila3)
		self.label_capital
		self.contenedor.attach_next_to(self.label_capital,self.lista_pasivos,Gtk.PositionType.BOTTOM,2,2)
	...
	def agregar_lista(self):
		...
		self.contenedor.attach_next_to(self.lista_activos,self.boton3,Gtk.PositionType.BOTTOM,2,2)
	...
	def agregar_fila3(self,btn):
		total_activo = 0
		for activo in self.modelo:
			total_activo += activo[1]

		total_pasivo = 0
		for pasivo in self.modelo2:
			total_pasivo += pasivo[1]

		capital =  total_activo - total_pasivo
        self.label_capital.set_text('Capital: {0}'.format(capital))
